...
def A1_SVM_ParameterTuning(x_train, y_train):
...
def A1_RF_ParameterTuning(x_train, y_train):
...
# RF modeling
def A1_RF(x_train,y_train):
    model = RandomForestClassifier(min_samples_split = 6, n_estimators = 200, max_depth = 4)   # Build the model
    model.fit(x_train, y_train)   # Fit the model
    return model
...
#title = r"Learning Curves (Task A1, SVM, poly kernel, C=10)"
#cv = 4
